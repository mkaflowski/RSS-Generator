...
def fileToItem(host, fname, pubDate, use_metadata=False):
    ...
    # print(host)
    # print(fname.encode('utf-8', 'replace').decode())
    fileURL = urllib.parse.quote(host.encode('utf-8', 'replace').decode() + fname.replace("\\", "/").encode('utf-8', 'replace').decode(), ":/")
    fileURL = fileURL.replace("/usr/home/kofii12345/domains/kofii12345.usermd.net/public_html/", "")
    fileURL = fileURL.replace(" ", "%20")
    fileURL = fileURL.replace("%3F%3F", "ę")
    # print(fileURL)
    ...
    try:
        ...
        try:
            ...
            parser.add_argument("-p", "--description", dest="description", help="Description of the podcast [Default:None]",
                                default=None, metavar="STRING")
            parser.add_argument("-a", "--author", dest="author", help="Author of the podcast [Default:None]",
                                default=None, metavar="STRING")
            ...
            if opts.sort_creation:
                ...
                try:
                    for fileName in fileNames:
                        ...
                        datetime_str = name.split(" ")[0]
                        # datetime_str = '09/19/18 13:55:26'

                        datetime_object = datetime.strptime(datetime_str, '%d.%m.%y')
                        pubDates[i] = datetime_object.timestamp()
                        i = i + 1
            ...
            # build items
            print(host)
            print(opts.use_metadata)
            items = [fileToItem(host, fname, pubDate, opts.use_metadata) for fname, pubDate in sortedFiles]

            if opts.outfile is not None:
                outfp = open(opts.outfile,"w",encoding='utf-8')
            else:
                outfp = sys.stdout

            outfp.write('<?xml version="1.0" encoding="UTF-8"?><rss xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:atom="http://www.w3.org/2005/Atom" version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:anchor="https://anchor.fm/xmlns">\n')
            outfp.write('   <channel>\n')
